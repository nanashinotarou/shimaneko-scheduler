...
- **外部API**: 
  - Google Gemini API (`gemini-2.5-flash`): スクショ画像からの高精度OCR＆シフト抽出
  - Notion API: SNS投稿スケジュールデータベースからのデータ取得
...
- **データ取得プロキシ**: Cloudflare Pages Functions (`/api/sns`) 経由でNotion APIへアクセスし、GASのCORSや認証制限を回避。
...
### 3.4 SNS投稿スケジュール同期機能 (SNS Mode)
- **Notion連携**: Notion上で管理されているInstagram・YouTubeの投稿スケジュールデータベース（「SNS投稿スケジュール」）の中身をAPI経由で動的に取得。
- **専用のカレンダービュー**: 画面上部のタブで「シフト管理モード」と「SNS更新モード」をシームレスに切り替え。シフトに埋もれることなく、SNSの投稿予定だけをすっきりカレンダー表示できます。
- **リッチな投稿バー**: Instagramは赤紫、YouTubeは赤色のテーマで表示。担当者絵文字（🍗 ササミ, 🐟 ツナ, 🐈‍⬛ コンブ）や最新のステータス（予定, 制作中, 投稿済み✅）を一目で確認できます。
- **高度なフィルタリング**: 「Instagramのみ」「投稿済みのみ」など、ワンタッチでカレンダー内の表示を切り替え可能。
- **セキュアなプロキシアーキテクチャ**: GAS（Google Apps Script）の `UrlFetchApp` 権限エラーを回避するため、Notion APIとの通信は **Cloudflare Pages Functions (`/api/sns`)** を経由。APIキーをフロントエンドに露出させずに安全な通信を実現しています。
...
- `functions/api/sns.js`: Cloudflare Pages Functions 用のバックエンドプロキシ（Notion API通信処理）
- `README.md` (Git用)
...
`;
        ...
        function renderCalendar() {
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                const apiDateKey = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
                const dailyData = grouped[dateKey] || [];
                const holidayName = holidaysData[apiDateKey];
                ...
                cell.innerHTML = `
                    <div class="date-header" style="justify-content: flex-start; gap: 4px; ${dateColorStr}">
                        <div style="${dateColorStr}">${d}</div>
